...
#define SP_ACTION_TITLE_FONTSIZE 16
...
- (void)setCustomSpacing:(CGFloat)spacing afterAction:(SPAlertAction *)action {
    if (@available(iOS 11.0, *)) {
        ...
        if (action.style == SPAlertActionStyleCancel) {
            MYLog(@"*** warning in -[SPAlertController setCustomSpacing:afterAction:]: 'the -action must not be a action with SPAlertActionStyleCancel style'");
        } else if (![self.otherActions containsObject:action]) {
            MYLog(@"*** warning in -[SPAlertController setCustomSpacing:afterAction:]: 'the -action must be contained in the -actions array, not a action with SPAlertActionStyleCancel style'");
        } else {
...
- (void)configureHeaderView {
    ...
    if (self.attributedMessage.length) {
        ...
        self.headerView.messageLabel.textAlignment = _textTitleAlignment;
        [self setupPreferredMaxLayoutWidthForLabel:self.headerView.messageLabel];
    }
}
...
- (void)setTextTitleAlignment:(NSTextAlignment)textTitleAlignment {
    _textTitleAlignment = textTitleAlignment;
    self.headerView.messageLabel.textAlignment = _textTitleAlignment;
}
