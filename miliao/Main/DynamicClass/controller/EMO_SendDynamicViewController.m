//
//  EMO_SendDynamicViewController.m
//  MeetHer
//
//  Created by 张世浩 on 2023/2/17.
//

#import "EMO_SendDynamicViewController.h"
...
@interface EMO_SendDynamicViewController ()
...
@implementation EMO_SendDynamicViewController
...
-(UITextView *)textView{
    if (!_textView) {
        _textView=[[UITextView alloc] init];
            _textView.text=getLanguage(@"  开心Share一下你的动态啊~");
...
- (UIButton *)sendBtn{
    if (!_sendBtn) {
        ...
        _sendBtn.frame = CGRectMake(0, 0, KAdaptedWidth(55), KAdaptedHeight(25));
...
//                  NSDictionary *dic = [[NSDictionary alloc] initWithDictionary:baseModel.data];
                  ...
                  if (wself.imgUrlArr.count==self->_selectedAssets.count) {
                      [wself.dataDic setObject:[wself.imgUrlArr componentsJoinedByString:@","] forKey:@"images"];
                      [self sendData];
                  }else{
...
-(void)upLoadData{
    NSLog(@"%@",self.dataDic);
    [NetworkRequest POST:Request_AddDynamic parmeters:self.dataDic success:^(id responObject) {
        NSLog(@"%@",responObject);
        BaseModel *baseModel = (BaseModel *)responObject;
        [SVProgressHUD dismiss];
