...
- (void)setModel:(id )model {
    ...
    if ([model isKindOfClass:[UIImage class]]){
        ...
        if ([aString rangeOfString:@"http"].location!=NSNotFound) {
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:aString] placeholderImage:[UIImage imageNamed:@"未加载图片"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                [self resizeSubviews];
            }];
        }else{
        ...
        [self.imageView sd_setImageWithURL:aURL placeholderImage:[UIImage imageNamed:@"未加载图片"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            [self resizeSubviews];
        }];
    }
