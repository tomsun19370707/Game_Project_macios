//
//  YJT_DynamicMsgTableViewCell.m
//  MeetHer
//
//  Created by 张世浩 on 2023/2/17.
//

#import "YJT_DynamicMsgTableViewCell.h"

@interface YJT_DynamicMsgTableViewCell()
...
@implementation YJT_DynamicMsgTableViewCell
...
-(void)setDicData:(NSDictionary *)dicData{
    _dicData=dicData;
    [self.headimgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",VERSION_HTTPS_SERVER,dicData[@"comment_avatar"]]] placeholderImage:KGetImage(@"未加载头像")];
    self.nickLabel.text=[NSString stringWithFormat:@"%@",dicData[@"comment_nickname"]];
    ...
    NSString *str=[Common isNull:dicData[@"lifes"][@"imgs"]];
    if (str.length>0) {//type类型:1=图片,2=视频
        ...
        if([dicData[@"lifes"][@"type"] integerValue]==2){
            ...
            [self.rightImgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",VERSION_HTTPS_SERVER,arr[0]]] placeholderImage:KGetImage(@"未加载头像")];
        }
    
    }else{
        self.dynamicLabel.hidden=NO;
        self.rightImgView.hidden=YES;
        self.dynamicLabel.text=[NSString stringWithFormat:@"%@",[Common isNull:dicData[@"lifes"][@"text"]]];
