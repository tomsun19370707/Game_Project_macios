...
// 下级控制器
#import "STAccountPhoneVc.h"
@interface STAccountSafeVc ()<UITableViewDelegate, UITableViewDataSource,DZNEmptyDataSetDelegate, DZNEmptyDataSetSource>
/** table */
...
@implementation STAccountSafeVc
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==0) {
        /** 手机号*/
        STAccountPhoneVc *ph = [[STAccountPhoneVc alloc]init];
        ph.vcType = 1 ;
...
{
    STAccountSafeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"STAccountSafeCell"];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"STAccountSafeCell" owner:self options:nil]lastObject];
    }
    if (indexPath.row==0) {
        cell.title.text = @"绑定手机号";
        cell.desc.text = @"188****1245";
    }else if (indexPath.row==1) {
        cell.title.text = @"密码";
        cell.desc.text = @"";
    }
...
-(TKBottomView *)bottomView
{
    if (!_bottomView) {
        ...
        [_bottomView.btn setTitle:@"注销账号" forState:UIControlStateNormal];
        [_bottomView.btn setTitleColor:HexColorDy(@"#999999") forState:UIControlStateNormal];
        [_bottomView.btn setBackgroundColor:HexColorDy(@"#F1F1F1") forState:UIControlStateNormal];
