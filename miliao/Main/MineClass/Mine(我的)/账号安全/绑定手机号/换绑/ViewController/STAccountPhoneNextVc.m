...
#import "STAccountPhoneNextVc.h"
// DTO

// View
#import "STAccountPhoneSms.h"
#import "STAccountPhoneNextCell.h"
// 下级控制器

@interface STAccountPhoneNextVc ()<UITableViewDelegate, UITableViewDataSource,DZNEmptyDataSetDelegate, DZNEmptyDataSetSource>
/** table */
...
@implementation STAccountPhoneNextVc
...
{
    if (indexPath.row==0) {
        STAccountPhoneNextCell *cell = [tableView dequeueReusableCellWithIdentifier:@"STAccountPhoneNextCell"];
        if (cell == nil) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"STAccountPhoneNextCell" owner:self options:nil]lastObject];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone ;
        return cell ;
    }
    
    STAccountPhoneSms *cell = [tableView dequeueReusableCellWithIdentifier:@"STAccountPhoneSms"];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"STAccountPhoneSms" owner:self options:nil]lastObject];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone ;
    return cell ;
}
...
#pragma mark --- 导航初始化
- (void)initNavBar {
    self.navigationBar.title = @"绑定手机号";
}
...
-(TKBottomView *)bottomView
{
    if (!_bottomView) {
        ...
        [_bottomView.btn setTitle:@"确认" forState:UIControlStateNormal];
