...
@interface TSMusicPlayerVc ()<UITableViewDelegate, UITableViewDataSource,DZNEmptyDataSetDelegate, DZNEmptyDataSetSource,MusicPlayToolsDelegate>
/** table */
...
@implementation TSMusicPlayerVc
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TSMusicPlayerInfo *cell = [tableView dequeueReusableCellWithIdentifier:@"TSMusicPlayerInfo"];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"TSMusicPlayerInfo" owner:self options:nil]lastObject];
    }
    cell.model = self.lookInfo ;
...
{
    if (!_listTableview) {
        _listTableview = [[UITableView alloc]initWithFrame:CGRectMake(0, NavBarHeight, SCREEN_WIDTH, SCREEN_HEIGHT_dy - self.oprVie.height - NavBarHeight) style:UITableViewStyleGrouped];
...
-(TSMusicPlayerOpr *)oprVie
{
    if (!_oprVie) {
        ...
        [_oprVie setFrame:CGRectMake(0, 0, SCREEN_WIDTH, _oprVie.contentView.height)];
        _oprVie.bottom = SCREEN_HEIGHT_dy ;
    }
    return _oprVie;
}
...
- (void)endOfPlayAction { 
    ...
    switch (self.oprVie.playMode) {
        case 1:
            {
                /** 循环播放*/
                self.playIndex++;
                if (self.playIndex >= self.playArr.count) {
                    self.playIndex = 0;
                }
        ...
        case 2:
            {
                /** 随机*/
                self.playIndex = arc4random() % self.playArr.count;
...
- (void)playMusic
{
    ...
    if (self.playIndex < self.playArr.count) {
        self.lookInfo = self.playArr[self.playIndex] ;
    }
