//
//  EMO_DressingCollectionViewCell.m
//  miliao
//
...
@interface EMO_DressingCollectionViewCell()
...
@implementation EMO_DressingCollectionViewCell
...
- (void)setModel:(PackModel *)model
{
    ...
    _model = model;
    NSString *uuid=[Common isNull:model.uuid];
    if(uuid.length<1){
        [self.nameBtn setTitle:[Common isNull:model.name] forState:UIControlStateNormal];
    }else{
        [self.nameBtn setTitle:uuid forState:UIControlStateNormal];
    }
    ...
    if(model.type==1){
        ...
        [self.nameBtn setImage:[UIImage imageNamed:@"liangIconImg"] forState:UIControlStateNormal];
...
//    [self.iconImage sd_setImageWithURL:[NSURL URLWithString:model.show_img]];
...
- (UIButton *)nameBtn{
    if (!_nameBtn) {
        ...
        [_nameBtn setTitle:getLanguage(@"00000") forState:UIControlStateNormal];
        [_nameBtn setTitleColor:RGBA(34, 34, 34, 1) forState:UIControlStateNormal];
        [_nameBtn setImage:[UIImage imageNamed:@"liangIconImg"] forState:UIControlStateNormal];
        _nameBtn.titleLabel.font=KFont(14);
        ...
        [_nameBtn setImagePositionWithType:SSImagePositionTypeLeft spacing:2];
    }
    return _nameBtn;
}
...
//            make.top.mas_equalTo(self.iconImage.mas_bottom).offset(KAdaptedHeight(15));
            make.top.mas_equalTo(KAdaptedHeight(5));
            make.leading.trailing.mas_equalTo(KAdaptedWidth(0));
            make.height.mas_equalTo(KAdaptedHeight(20));
...
//            make.top.mas_equalTo(self.nameLabel.mas_bottom).offset(KAdaptedHeight(0));
            make.top.mas_equalTo(self.nameBtn.mas_bottom).offset(KAdaptedHeight(5));
            make.leading.trailing.mas_equalTo(KAdaptedWidth(0));
            make.height.mas_equalTo(KAdaptedHeight(20));
...
- (UILabel *)defaultLabel{
    if (!_defaultLabel) {
        ...
        [_defaultLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.priceLabel.mas_bottom).offset(KAdaptedHeight(0));
            make.height.mas_equalTo(KAdaptedHeight(20));
            make.leading.trailing.mas_equalTo(KAdaptedWidth(0));
