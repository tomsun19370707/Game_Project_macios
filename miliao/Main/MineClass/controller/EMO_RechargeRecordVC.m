...
#import "EMO_RechargeRecordVC.h"
#import "EMO_RechargeRecordCell.h"
@interface EMO_RechargeRecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation EMO_RechargeRecordVC
...
- (void)viewDidLoad {
    ...
    if(self.type==1){
        self.titleLabel.text=getLanguage(@"充值明细");
    }else{
        self.titleLabel.text=getLanguage(@"提现明细");
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    EMO_RechargeRecordCell *cell=[tableView dequeueReusableCellWithIdentifier:[NSString stringWithFormat:@"Cell"]];
    if (!cell) {
        cell=[[EMO_RechargeRecordCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:[NSString stringWithFormat:@"Cell"]];
    }
...
- (void)reuqestList:(NSString *)time andFresh:(BOOL)fresh{
    ...
    [NetworkRequest POST:self.type==1?Request_MoneyRecordList:Request_GetMyWithdrawal parmeters:dic success:^(id responObject) {
