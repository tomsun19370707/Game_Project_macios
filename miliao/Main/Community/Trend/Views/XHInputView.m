...
#define XHInputView_StyleDefault_Height 50
...
-(void)setupStyleDefaultUI{
    ...
    [_sendButton setTitleColor:mainQianColor forState:UIControlStateDisabled];
    [_sendButton setTitleColor:MLControlsColor forState:UIControlStateNormal];
    _sendButton.enabled = NO;
    [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
    [_sendButton addTarget:self action:@selector(sendButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    _sendButton.titleLabel.font = [UIFont systemFontOfSize:15];
    _sendButton.backgroundColor = BaseMainColor;
    [_sendButton makeRoundCorner];
...
-(void)setupStyleLargeUI{
    ...
    _sendButton.backgroundColor = BaseMainColor;
    _sendButton.titleLabel.textColor = MHColorFromHexString(@"#999999");
    [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
    ...
    if(_style == InputViewStyleDefault){
        ...
        if(height >= heightDefault){
            [UIView animateWithDuration:0.3 animations:^{
                //调整frame
                CGRect frame = weakSelf.showFrameDefault;
                frame.size.height = height;
                frame.origin.y = weakSelf.showFrameDefault.origin.y - (height - weakSelf.showFrameDefault.size.height);
                self.frame = frame;
                
                //调整sendButton frame
                weakSelf.sendButton.frame = CGRectMake(XHInputView_ScreenW - XHInputView_StyleDefault_LRSpace - weakSelf.sendButton.frame.size.width, self.bounds.size.height - weakSelf.sendButton.bounds.size.height - XHInputView_StyleDefault_TBSpace, weakSelf.sendButton.bounds.size.width, weakSelf.sendButton.bounds.size.height);
...
- (void)keyboardWillAppear:(NSNotification *)noti{
    ...
    if(_textView.isFirstResponder){
        ...
        MYLog(@"keyboardSize.height = %f",keyboardSize.height);
...
-(void)setSendButtonColor:(UIColor *)sendButtonColor{
    ...
    [_sendButton setTitleColor:sendButtonColor forState:UIControlStateNormal];
}
