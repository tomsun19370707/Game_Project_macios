...
+ (void)getPrizesWithTypeId:(NSInteger)typeId 
                    ...
                    failure:(void(^)(NSError *error))failure {
    [self getPrizesWithTypeId:typeId successWithInfo:^(NSArray<MLGameDrawResultModel *> *list, NSInteger luckyValue, NSInteger luckyLimit, MLGameLotteryInfoModel * _Nullable infoModel) {
...
+ (void)getPrizesWithTypeId:(NSInteger)typeId 
    ...
    if (typeId == 11) {
        ...
        [MLNetWorkHelper GET:url parameters:[self buildParams:params] success:^(id responseObject) {
            ...
            if ([responseObject[@"code"] integerValue] == 1) {
                ...
                MLGameLotteryInfoModel *infoModel = nil;
                if ([rawData isKindOfClass:[NSDictionary class]]) {
                    infoModel = [MLGameLotteryInfoModel mj_objectWithKeyValues:rawData];
                ...
                if (success) success(list, luckyVal, luckyLim, infoModel);
            } else {
                if (failure) {
                    NSError *error = [NSError errorWithDomain:@"MLGameLotteryServiceErrorDomain" 
                                                         code:[responseObject[@"code"] integerValue] 
    ...
    [MLNetWorkHelper POST:url parameters:[self buildParams:params] success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 1) {
            ...
            if (success) success(list, -1, 200, nil);
        } else {
            if (failure) {
                NSError *error = [NSError errorWithDomain:@"MLGameLotteryServiceErrorDomain" 
                                                     code:[responseObject[@"code"] integerValue] 
...
+ (void)refreshPoolWithTypeId:(NSInteger)typeId 
                      ...
                      failure:(void(^)(NSError *error))failure {
    [self refreshPoolWithTypeId:typeId successWithInfo:^(NSArray<MLGameDrawResultModel *> *list, NSInteger diamondCost, NSString *newDiamondBalance, NSInteger luckyValue, NSInteger luckyLimit, MLGameLotteryInfoModel * _Nullable infoModel) {
    ...
    [MLNetWorkHelper POST:url parameters:[self buildParams:params] success:^(id responseObject) {
        ...
        if ([responseObject[@"code"] integerValue] == 1) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                infoModel = [MLGameLotteryInfoModel mj_objectWithKeyValues:data];
            ...
            if (success) success(list, cost, balance, luckyVal, luckyLim, infoModel);
        } else {
            if (failure) {
                NSError *error = [NSError errorWithDomain:@"MLGameLotteryServiceErrorDomain" 
                                                     code:[responseObject[@"code"] integerValue] 
