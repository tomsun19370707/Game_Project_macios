...
// ==========================================
// MLChatRoomThemeGameOneRecordCell (记录行单元格)
// ==========================================
...
- (void)configureWithData:(NSDictionary *)data isMine:(BOOL)isMine isExpanded:(BOOL)expanded {
    ...
    _summaryLabel.text = [NSString stringWithFormat:@"%@抽 (%@钻石)", data[@"draw_times"], data[@"total_value"]];
...
// ==========================================
// MLChatRoomThemeGameOneRecordView
// ==========================================
...
@property (nonatomic, strong) NSMutableSet<NSNumber *> *expandedRowIds; // 记录展开的行 ID 或行 Index
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (expanded) {
        NSDictionary *data = self.recordList[indexPath.row];
        NSArray *items = data[@"items"];
        NSInteger rows = (items.count + 3) / 4; // 每排4个礼物
