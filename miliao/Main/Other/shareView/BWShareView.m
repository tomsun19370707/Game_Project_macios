...
- (void)show
{
    ...
    [UIView animateWithDuration:.3f animations:^{
        ...
        frame.origin.y = [self isDoubleShare] ? [UIScreen mainScreen].bounds.size.height - 230 - kTitleHeight : [UIScreen mainScreen].bounds.size.height - KAdaptedHeight(140) - kTitleHeight;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self removeChildView];
//    [self performSelector:@selector(removeChildView) withObject:nil afterDelay:1];
...
{
    if (!_bgView) {
        CGFloat bgH = KAdaptedHeight(160);
        if ([self isDoubleShare]) {
            bgH = 230.0;
        }
        ...
        _bgView.layer.cornerRadius=KAdaptedHeight(24);
...
{
    if (!_shareTitleLabel) {
        _shareTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(KAdaptedWidth(25), 0,kWidth-KAdaptedWidth(50), kTitleHeight)];
        _shareTitleLabel.text = [NSString stringWithFormat:@"%@", self.shareTitle];
        _shareTitleLabel.backgroundColor =kWhiteColor;
        _shareTitleLabel.textColor = RGBA(51, 51, 51, 1);
//        _shareTitleLabel.textAlignment = NSTextAlignmentLeft;
        _shareTitleLabel.textAlignment = NSTextAlignmentCenter;
        _shareTitleLabel.font = KFont(14);
...
- (UICollectionViewFlowLayout *)layout
{
    if (!_layout) {
        ...
        // 两种布局
        // 1、单行显示所有的分享类型
        _layout.itemSize = CGSizeMake(kWidth/self.shareArray.count-KAdaptedWidth(10),KAdaptedHeight(70));
        // 2、两行甚至多行显示的分享类型
...
- (UIButton *)cancleButton
{
    if (!_cancleButton) {
        ...
        _cancleButton.frame = CGRectMake(0, CGRectGetMaxY(self.collectionView2.frame), [UIScreen mainScreen].bounds.size.width, KAdaptedHeight(50));
        _cancleButton.backgroundColor = UIColor.whiteColor;
        [_cancleButton setTitle:getLanguage(@"取消") forState:UIControlStateNormal];
        [_cancleButton setTitleColor:RGBA(153, 153, 153, 1) forState:UIControlStateNormal];
        _cancleButton.titleLabel.font = KFont(12);
