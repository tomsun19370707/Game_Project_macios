...
- (instancetype)initWithCustomView:(UIView *)customView andLeft:(CGFloat)Left andRight:(CGFloat)Right andShadow:(BOOL)shadows
{
...
//        [self performSelector:@selector(setupUI)];
        [self setupUI:Left andRight:Right andlayershadow:shadows];
    }
    return self;
}
...
-(instancetype)ds_showTitle:(NSString *)title message:(NSString *)message image:(UIImage *)image buttonTitles:(NSArray *)buttonTitles buttonTitlesColor:(NSArray<UIColor *> *)buttonTitlesColor Frame:(NSInteger)AAA{
...
//    if (self == [super initWithFrame:CGRectMake(0, 0, kDSAlertWidth, 0)]){
    if (self == [super initWithFrame:CGRectMake(0, 0, self.viewWidth - 150, 0)]){
        _title             = [title copy];
        _image             = image;
...
- (void)setupUI:(CGFloat)Left andRight:(CGFloat)right andlayershadow:(BOOL)shadows
{
    self.viewWidth                   = SCREENWIDTH;
    self.viewHeight                  = SCREENHEIGHT;
    ...
    if ( !self.subView.translatesAutoresizingMaskIntoConstraints ) {
        ...
        if (Left==0||right==0) {
            ...
            [self.subView autoPinEdgeToSuperviewEdge:ALEdgeLeft withInset:Left relation:NSLayoutRelationEqual];
                        [self.subView autoPinEdgeToSuperviewEdge:ALEdgeRight withInset:right relation:NSLayoutRelationEqual];
        }
        
       
//        [self.subView autoPinEdgeToSuperviewEdge:ALEdgeLeft withInset:isplus?150:100 relation:NSLayoutRelationEqual];
//        [self.subView autoPinEdgeToSuperviewEdge:ALEdgeRight withInset:isplus?150:100 relation:NSLayoutRelationEqual];
...
+ (void)ds_showCustomView:(UIView *)customView
            configuration:(void (^)(DSAlertChange *tempView)) configuration
{
//    DSAlertChange *temp = [[DSAlertChange alloc] initWithCustomView:customView];
    DSAlertChange *temp = [[DSAlertChange alloc] initWithCustomView:customView andLeft:30 andRight:30 andShadow:YES];
    if (configuration)
    {
        configuration(temp);
    }
...
{
    DSAlertChange *temp = [[DSAlertChange alloc] ds_showTitle:title
                                          message:message
                                            image:image
